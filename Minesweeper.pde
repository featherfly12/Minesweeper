...
public void mousePressed () {
    if (mouseButton == RIGHT) { // Right-click to toggle flag
        marked = !marked;
        return; // Prevent normal click behavior
    }

    if (marked) return; // Don't allow clicking flagged buttons
...
public void draw () {  
    if (isLose && bombs.contains(this)) {
        fill(255, 0, 0); // Turn all bombs red on loss
    }
    else if (marked) {
        fill(0); // Black for flagged tiles
    }
    else if (clicked) {
        fill(225, 242, 167); // Revealed tile color
    }
    else { 
        fill(170, 215, 81); // Default tile color
    }
    rect(x, y, width, height);
    
    if (marked) { // Draw a small red triangle in flagged squares
        fill(255, 0, 0);
